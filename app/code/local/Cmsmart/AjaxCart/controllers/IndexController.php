...
class Cmsmart_AjaxCart_IndexController extends Mage_Core_Controller_Front_Action{
	...
	public function indexAction(){
		...
		foreach( $_GET as $key => $value ){
			if( is_array($value) ) $requestData[$filter->filter($key)] = $value;
			else $requestData[$filter->filter($key)] = $filter->filter($value);
		}
		...
		if(!$delete){
			...
			if($kiemtra) {
				...
				self::tryaddAction($product, $requestData);
			}
		...
		<div>
			<ul class="messages ajaxcart-messages">
				<li class="success-msg">
					<ul>
						<li><span><a title="'.@$product->getName().'" href="'.@$product->getProductUrl().'">'.@$product->getName().' </a><br/>'.$this->__('foi adicionado ao seu carrinho').'.</span></li>
						<li>
							<a title="Fechar" class="continue_shopping">Continuar Comprando</a>
							<button style="text-align: right;" onclick="setLocation(\''.Mage::getBaseUrl().'checkout/\')" class="button btn-continue" title="'.$this->__('Checkout').'" type="button"><span><span>'.$this->__('Finalizar compra').'</span></span></button>
						</li>
					</ul>
				</li>
			</ul>
...
//		}
	...
	public function _getProductFromUrl(){
		...
		if(!$product_id):
			// Zend_Debug::dump($this->getRequest()->getParam('product'));
