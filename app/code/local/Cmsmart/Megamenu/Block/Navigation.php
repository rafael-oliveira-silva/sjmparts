   ...
   protected function _renderCategoryMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false,
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if ($childrenWrapClass) {
            ...
            if($level == 0) {
             if(!empty($megamenu[0]) && $megamenu[0]['active_product']==1){
            ...
            $html[] = $this->getBlockRight($megamenu);
              ...
              if(!empty($megamenu[0]) && $megamenu[0]['category_children']==0)   {
                if($htmlChildren) {
                    if($level == 0){
		...
		if($isLast && $level == 1){
			// echo $level;
			$html[] .= '<li class="level1 nav-1-11 last item  no-level-thumbnail">
							<a style="background-color:#008B23;border-right: 1px solid #999;" href="'.Mage::getBaseUrl().'promocoes.html" class="">
								<span>Promoções</span>
							</a>
						</li>';
			// $html[] = '';
    ...
    public function getBlockTop($block){
		if(!empty($block[0])){
			...
			$active_static_block 		= array();
			$active_static_block_top 	= array();
			$idblock 					= array();
		}
    ...
    public function getBlockLeft($block){
		if(!empty($block[0])){
			...
			$active_static_block 		= array();
			$active_static_block_left 	= array();
			$id 						= array();
			$idblock 					= array();
		}
    ...
    public function getBlockRight($block){
		if(!empty($block[0])){
			...
			$active_static_block = array();
			$active_static_block_right = array();
			$idblock = array();
			$id = array();
		}
    ...
    public function getBlockBottom($block){
		if(!empty($block[0])){
			...
			$active_static_block = array();
			$active_static_block_bottom = array();
			$idblock = array();
		}
    ...
    public function widthp($idc){
        ...
        $width_menu = Mage::getStoreConfig('megamenu/mainmenu/width_menu');
                ...
                if(!empty($width[0]) && ($width[0]['active_static_block'] == 1 ) && ($width[0]['active_static_block_left'] == 1 ) && ($width[0]['active_static_block_right'] == 1 )) {
                        $widthchildrenli = $width_menu -  $width[0]['width_block_left'] -  $width[0]['width_block_right']-30-10;
                    } else {
                           if(!empty($width[0]) && ($width[0]['active_static_block'] == 1 )&& ($width[0]['active_static_block_left'] == 1 ) && ($width[0]['active_static_block_right'] == 0 )) {
                                $widthchildrenli = $width_menu -  $width[0]['width_block_left']-(30*$column)-30;
                            } else {
                                if(!empty($width[0]) && ($width[0]['active_static_block'] == 1 )&& ($width[0]['active_static_block_left'] == 0 ) && ($width[0]['active_static_block_right'] == 1 )) {
    ...
    public function getLabel($megamenu){
         if(!empty($megamenu[0]) && $megamenu[0]['active_label'] == 1){
    ...
    public function getLevle($id){
        $megamenu = Mage::helper('megamenu')->Megamenu($id);
         if(!empty($megamenu[0]) && $megamenu[0]['level_column_count'] != 0){
    ...
    public function fontcolor($megamenu){
        if(!empty($megamenu[0]) && $megamenu[0]['font-color']) {
    ...
    public function backgroundcolor($megamenu){
        if(!empty($megamenu[0]) && $megamenu[0]['background-color']) {
