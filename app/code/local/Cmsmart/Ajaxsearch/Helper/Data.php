...
class Cmsmart_Ajaxsearch_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function str_highlight($text, $needle, $options = null, $highlight = null)
    {
        ...
        if ($options & self::STR_HIGHLIGHT_SIMPLE) {
            $pattern = '#(%s)#';
            $sl_pattern = '#(%s)#';
        } else {
            $pattern = '#(?!<.*?)(%s)(?![^<>]*?>)#';
        ...
        // Case sensitivity
        if (!($options & self::STR_HIGHLIGHT_CASESENS)) {
            $pattern .= 'i';
            $sl_pattern .= 'i';
        }
        ...
        foreach ($needle as $needle_s) {
            ...
            if ($options & self::STR_HIGHLIGHT_WHOLEWD) {
                $needle_s = '\b' . $needle_s . '\b';
            }
     
            // Strip links
            if ($options & self::STR_HIGHLIGHT_STRIPLINKS) {
