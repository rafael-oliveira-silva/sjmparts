...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function createPostAction()
    {
        ...
        // Validate Recaptcha
        $recaptchaChallenge = $this->getRequest()->getParam('g_recaptcha');
        if (empty($recaptchaChallenge)) {
            $errUrl = $this->_getUrl('*/*/create', array('_secure' => true));
            $this->_redirectError($errUrl);
            $this->_addSessionError('Recaptcha inválido');
            return;
        }

        $secretKey = Mage::getStoreConfig('webforms/captcha/private_key');
        $response = file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret={$secretKey}&response={$recaptchaChallenge}");
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                $this->_getSession()->setCustomer($customer)
                    ...
                    ->addSuccess('A informação da conta foi salva com sucesso. Para prosseguir com a sua compra, <a href="'.Mage::helper('checkout/cart')->getCartUrl().'">clique aqui</a>');
