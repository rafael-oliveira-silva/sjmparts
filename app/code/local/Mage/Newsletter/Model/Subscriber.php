...
class Mage_Newsletter_Model_Subscriber extends Mage_Core_Model_Abstract
{
	...
	public function subscribe($email, $name)
	{
		...
		if(empty($name)){
			$name = Mage::app()->getRequest()->getParam('firstname');
		}
		// Zend_Debug::dump($_POST);
		// Zend_Debug::dump(Mage::app()->getRequest()->getParam('firstname'));
		// Zend_Debug::dump($name);
		// exit;
		$personType = Mage::app()->getRequest()->getParam('person_type');
		if(!empty($personType)){
			if($personType == 1){
		...
		if (!$this->getId() || $this->getStatus() == self::STATUS_UNSUBSCRIBED
			...
			$this->setData('subscriber_firstname', $name);
		...
		try {
			// Zend_Debug::dump(get_class($this->save()));
