...
class PagSeguro_Model_Standard extends Mage_Payment_Model_Method_Abstract {
        ...
        if ($items) {
            ...
            foreach($items as $item) {
                ...
                $sArr = array_merge($sArr, array(
                        ...
                        'item_valor_'.$i   => (($item_price/100)*$this->getConfigData('taxa_pag_seguro'))+$item_price,
                ));
                $i++;
            }
            ...
            if(!$active){
                if ($shipping_amount > 0) {
                    ...
                    $sArr = array_merge($sArr, array(
                        ...
                        'item_id_'.$i      => "frete",
                        'item_quant_'.$i   => 1,
                        'item_valor_'.$i   => (($shipping_amount/100)*$this->getConfigData('taxa_pag_seguro'))+$shipping_amount,
                    ));
                    $i++;
                }
            }
	...
	public function checkAbandonedCarts(){
		...
		//Variáveis do cURL
		$cURL = curl_init();
		$initialDate = str_replace( ' ', 'T', date( 'Y-m-d H:i', strtotime("-3 hours -1 minute") ) ); //O script roda a cada 3 horas, por segurança adicionamos 1 minuto para o possível delay
