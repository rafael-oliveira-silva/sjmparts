...
class Multikomerce_Redecard_PayController extends Mage_Core_Controller_Front_Action
{
  ...
  public function getOrder()
	...
	$payment = $order->getPayment(); //pega os dados do pagamento
     ...
     /*echo $total ."<br>";
        ...
        $additionaldata['autorizacao'] = array(
                        ...
                        'numsqn' => (string) $autorizacao['numsqn'],
                        'cc_parcelas' => $parcelas
				);
        $payment->setAdditionalData(serialize($additionaldata));
        $payment->save();

        // if($_SERVER['REMOTE_ADDR'] == '201.6.233.203'){
        //     ini_set('display_errors', true);
        //     Zend_Debug::dump($autorizacao);
        //     Zend_Debug::dump('teste');
        //     exit;
            if(empty($autorizacao)){
                if(!isset($additionaldata['autorizacao']) || empty($additionaldata['autorizacao'])){
                    $order = Mage::getModel('sales/order')->load($orderId);
                    if( !is_null($order->getId()) ){
                        try{
                            $order->cancel();
                            $order->addStatusToHistory($order->getStatus(), 'Código e mensagem da transação: custom99 - Transação não concluída, tente novamente.');
                            $order->save();
                        }catch(Exception $e){
                    ...
                    $this->_redirect('komerci/pay/failure/codigo/99/motivo/TRANSACAO+NAO+CONCLUIDA+TENTE+NOVAMENTE/orderid/'.$orderId);
                }
            }
        // }
        ...
        if($autorizacao['codret'] == 0)
        {
            ...
            if( !is_null($order->getId()) ){
                try{
                    $order->cancel();
                    $order->addStatusToHistory($order->getStatus(), 'Código e mensagem da transação: '.utf8_encode(urldecode($autorizacao['codret'])).' - '.utf8_encode(urldecode($autorizacao['msgret'])));
                    $order->save();
                }catch(Exception $e){
                    Mage::log($e->getMessage());
                }
    ...
    public function successAction()
    {
        ...
        $motivo = utf8_encode($this->getRequest()->getParam('motivo'));
