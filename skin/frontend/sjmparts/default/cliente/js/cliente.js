...
function consultarCep(obj){
	...
	var _baseUrl = baseUrl + 'cliente/cep/consultar/cep/';
	var cep = target.find('div.field.zip input').val().replace('-','');	
	
	var url = _baseUrl + cep;
	...
	jQuery.getJSON(url, function(data){
		...
		if( data.cidade != '[object Object]' ){
			...
			target.find('div.field.region select option').each(function(){
				...
				if( this.hasAttribute('data-uf') && this.getAttribute('data-uf').toUpperCase() == data.uf ) jQuery(this).attr('selected','selected');
			});
